# GamFu — فایل نصب gamerFuz

این مخزن **فقط فایل نصب** است. سورس کد اینجا نیست.

## دانلود مستقیم

| پلتفرم | نسخه | دانلود |
| --- | --- | --- |
| اندروید | 1.0.8 | [**دانلود APK**](https://raw.githubusercontent.com/ar2va/GamFu/main/downloads/gamerFuz-android.apk) |
| ویندوز | — | به‌زودی |

لینک اندروید با یک کلیک فایل `gamerFuz-android.apk` را مستقیم دانلود می‌کند (بدون نیاز به لاگین GitHub).

پس از نصب، اپ نسخهٔ جدید را از API ادمین هم چک می‌کند.
